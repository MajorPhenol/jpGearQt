...
### Stand-alone program
jpGear has been compiled into a stand-alone program for a couple platforms using [nuitka](https://nuitka.net/).  Go to the `dist/` folder and find your platform.  Download and run the executable, no installation required.

### Run the python script directly
If you have python installed, you can also run the jpGear script directly.
```
...
Most beginner gear design literature will prescibe certain tooth proportions, such as the height of the tooth addendum being *1\*module*.  These proportions are then used to calculate the actual tooth dimensions (in the case of the addendum, this determines the outer diameter of the gear).  jpGear instead allows you to set the tooth dimension directly.  jpGear will show you certain "standard" dimensions, but gives you the freedom to tweak them.

Additonally, the beginner's approach is to create a pair of gears first, then check for any interferences in the mesh.  In practice, it is usually more helpful to design gears based around minimum clearance constraints.  jpGear lets you assign things like *backlash* and *root clearance*, and will create tooth geometry that fits your constraints.

## A Note About Internal Gear Terminology
jpGear supports the creation of internal gears.  The geometry of an internal gear is generated by first creating an external gear, then removing this geometry from the center of the internal gear.  Thus the "tooth" of an internal gear has the same geometry as the gap between teeth on an external gear.  jpGear labels the dimensions of the internal gear based on the generating external gear.  For example, the inner-most radius of the internal gear is labeled the *root radius* (because it has the same geometry as the *root radius* of an external gear), whereas other sources might call this the *tip radius*.

The exception to this is the tooth thickness, which measures the actual material thickness and not the gap left by the generating external gear tooth.
...
The module sets the overall scale of the gear.  So for example, doubling the module will double the size of the gear.  **Two mating gears must have the same module.**  In theory, the module can be anything you like.  But practically speaking, manufacturers stick to certain common sizes.  Here's a partial list of common (metric) sizes:
